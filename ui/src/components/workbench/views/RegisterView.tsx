...
export const RegisterView: React.FC = () => {
  ...
  const dynamicStages: StageDetail[] = [
    ...
    },
    {
      ...
      icon: Crosshair,
      description: 'Inverse-Compositional Lucas-Kanade 21×21 sub-pixel refinement with ECC fallback cascade.',
      accentColor: '#22d3ee',
      badgeBg: 'rgba(34, 211, 238, 0.15)',
      kpis: [
        { label: 'ENGINE', value: 'IC-LK 21×21 + ECC Cascade', color: '#22d3ee' },
        { label: 'CONVERGENCE EPS', value: '‖Δp‖ < 0.01 px' },
        { label: 'STATUS', value: isComplete ? (results.rmse < 1.0 ? '✓ Sub-Pixel Certified' : 'Review Required') : 'Pending Run', color: '#34d399' },
        { label: 'TARGET', value: '< 1.0 px Sub-Pixel' },
      ],
    },
    {
      id: '08',
      ...
      kpis: [
        { label: 'TRAIN RMSE', value: results.rmse > 0 ? `${results.rmse.toFixed(3)} px` : 'Pending Run', color: '#fbbf24' },
        { label: 'VAL HOLDOUT RMSE', value: results.rmseVal != null ? `${results.rmseVal.toFixed(3)} px` : (results.rmse > 0 ? 'Evaluated' : 'Pending Run') },
        { label: 'COVERAGE INDEX', value: results.coverage > 0 ? `${results.coverage}% Spatial Mesh` : 'Pending Run' },
        { label: 'UNIFORMITY NNI', value: results.nni > 0 ? `${results.nni.toFixed(3)}` : 'Pending Run' },
      ],
    },
    {
      id: '09',
